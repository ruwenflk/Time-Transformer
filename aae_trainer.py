    ...
    def on_epoch_end(self, epoch, logs=None):
        if (epoch + 1) % 50 == 0:
            ...
            self.model.save_weights(
                self.path + "/checkpoint" + str(ckp_num) + "/aae_ckp"
            )
...
if __name__ == "__main__":
    ...
    ae_schedule = PolynomialDecay(
        initial_learning_rate=0.005,
        decay_steps=300,
        end_learning_rate=0.0025,
        power=0.5,
    )
    dc_schedule = PolynomialDecay(
        initial_learning_rate=0.001,
        decay_steps=300,
        end_learning_rate=0.0001,
        power=0.5,
    )
    ge_schedule = PolynomialDecay(
        initial_learning_rate=0.001,
        decay_steps=300,
        end_learning_rate=0.0001,
        power=0.5,
    ...
    history = model.fit(
        x_train, epochs=500, batch_size=batch_size, callbacks=[cbk], verbose=0
    )
    ...
    arr_chunks = sample_dataset(
        n_samples=10000, batch_size=batch_size, trained_model=model
    )
